...
internal partial class ArknightsGameInstaller
{
    private sealed class Install
    {
        private readonly ArknightsGameInstaller _owner;

        public Install(ArknightsGameInstaller owner)
        {
            ...
            InstallProgressStateDelegate? progressStateDelegate, CancellationToken token)
        {
            ...
            if (_owner.GameManager is not ArknightsGameManager manager || manager.GamePacks == null ||
            ...
            var downloadTasks = new List<ArknightsPack>();
        ...
        {
            SharedStatic.InstanceLogger.LogInformation($"[ArknightsInstaller] Preparing decompression (Virtual merge stream): {sourceDir} -> {destDir}");
            ...
            if (partFiles.Count == 0)
            {
                SharedStatic.InstanceLogger.LogError("[ArknightsInstaller] No archive files found!");
                throw new FileNotFoundException("No archive found in Downloads folder");
            }

            SharedStatic.InstanceLogger.LogInformation($"[ArknightsInstaller] Found {partFiles.Count} split volume files.");
            ...
            await Task.Run(async () =>
            {
                try
                {
                    ...
                    try
                    {
                        SharedStatic.InstanceLogger.LogInformation("[ArknightsInstaller] Starting extraction...");
                    ...
                    SharedStatic.InstanceLogger.LogInformation("[ArknightsInstaller] Decompression complete!");
                }
                catch (Exception ex)
                {
                    SharedStatic.InstanceLogger.LogError($"[ArknightsInstaller] Decompression failed: {ex}");
                    throw;
                }
            }, token);
        }
