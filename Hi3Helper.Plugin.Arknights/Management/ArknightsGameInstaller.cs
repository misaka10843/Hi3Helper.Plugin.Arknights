...
[GeneratedComClass]
internal partial class ArknightsGameInstaller : GameInstallerBase
{
    private readonly HttpClient _downloadHttpClient;

    internal ArknightsGameInstaller(IGameManager? gameManager) : base(gameManager)
    {
    ...
    {
        if (GameManager is not ArknightsGameManager arknightsManager)
            throw new InvalidOperationException("GameManager is not ArknightsGameManager");

        return await arknightsManager.InitAsyncInner(true, token).ConfigureAwait(false);
    }
        ...
        CancellationToken token)
    {
        ...
        if (GameManager is not ArknightsGameManager { GamePacks: { } packs }) return 0L;
    ...
    protected override Task UninstallAsyncInner(CancellationToken token)
    {
        ...
        {
            SharedStatic.InstanceLogger.LogError($"[Arknights] Uninstall failed: {ex.Message}");
        }
