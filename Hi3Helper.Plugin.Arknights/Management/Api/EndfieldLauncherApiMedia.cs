...
[GeneratedComClass]
public partial class ArknightsLauncherApiMedia : LauncherApiMediaBase
{
    ...
    private ArknightsGetMainBgImageRsp? _bgResponse;

    public ArknightsLauncherApiMedia(string webApiUrl, string appCode, string channel, string subChannel, string seq)
    {
    ...
    protected override async Task<int> InitAsync(CancellationToken token)
    {
        var requestBody = new ArknightsBatchRequest
        {
            ...
            ProxyReqs = new[]
            {
                new ArknightsProxyRequest { Kind = "get_main_bg_image", GetMainBgImageReq = CreateCommonReq() }
            }.ToList()
        };
        ...
        try
        {
            ...
            SharedStatic.InstanceLogger.LogDebug($"[ArknightsMedia] Request Body:\n{jsonRequest}");

            using var content = new StringContent(jsonRequest, Encoding.UTF8, "application/json");
            using var response = await ApiResponseHttpClient.PostAsync(_webApiUrl, content, token);

            SharedStatic.InstanceLogger.LogDebug($"[ArknightsMedia] API Response Code: {response.StatusCode}");
            response.EnsureSuccessStatusCode();

            var jsonResponse = await response.Content.ReadAsStringAsync(token);
            SharedStatic.InstanceLogger.LogDebug($"[ArknightsMedia] Response Body:\n{jsonResponse}");
            ...
            SharedStatic.InstanceLogger.LogDebug(
                $"[ArknightsMedia] Background URL: {_bgResponse?.MainBgImage?.Url}, Video: {_bgResponse?.MainBgImage?.VideoUrl}");
            return 0;
        }
        catch (Exception ex)
        {
            SharedStatic.InstanceLogger.LogError($"[ArknightsMedia] Failed to init media: {ex}");
            return -1;
        }
    }
        ...
        out bool isAllocated)
    {
        ...
        SharedStatic.InstanceLogger.LogDebug(
            $"[ArknightsMedia] Background image: {_bgResponse?.MainBgImage?.Url}, Background video: {_bgResponse?.MainBgImage?.VideoUrl}");
    ...
    {
        SharedStatic.InstanceLogger.LogDebug($"[ArknightsMedia] Downloading background: {fileUrl}");
        try
        {
            await base.DownloadAssetAsyncInner(ApiDownloadHttpClient, fileUrl, outputStream, fileChecksum,
                downloadProgress, token);
            SharedStatic.InstanceLogger.LogDebug($"[ArknightsMedia] Background download COMPLETED.");
        }
        catch (Exception ex)
        {
            SharedStatic.InstanceLogger.LogError(
                $"[ArknightsMedia] Background download FAILED: {fileUrl}\nException: {ex}");
        }
    }
