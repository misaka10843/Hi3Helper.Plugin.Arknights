...
[GeneratedComClass]
public partial class ArknightsLauncherApiNews : LauncherApiNewsBase
{
    ...
    private ArknightsGetBannerRsp? _bannerResponse;
    private ArknightsGetAnnouncementRsp? _newsResponse;
    private ArknightsGetSidebarRsp? _sidebarResponse;

    public ArknightsLauncherApiNews(string webApiUrl, string appCode, string channel, string subChannel, string seq)
    {
    ...
    protected override async Task<int> InitAsync(CancellationToken token)
    {
        var requestBody = new ArknightsBatchRequest
        {
            ...
            ProxyReqs = new[]
            {
                new ArknightsProxyRequest { Kind = "get_announcement", GetAnnouncementReq = CreateCommonReq() },
                new ArknightsProxyRequest { Kind = "get_banner", GetBannerReq = CreateCommonReq() },
                new ArknightsProxyRequest { Kind = "get_sidebar", GetSidebarReq = CreateCommonReq() }
            }.ToList()
        };

        try
        {
            var jsonRequest = JsonSerializer.Serialize(requestBody, ArknightsApiContext.Default.ArknightsBatchRequest);
            SharedStatic.InstanceLogger.LogDebug($"[ArknightsNews] Request Body:\n{jsonRequest}");

            using var content = new StringContent(jsonRequest, Encoding.UTF8, "application/json");
            using var response = await ApiResponseHttpClient.PostAsync(_webApiUrl, content, token);

            SharedStatic.InstanceLogger.LogDebug($"[ArknightsNews] API Response Code: {response.StatusCode}");
            response.EnsureSuccessStatusCode();

            var jsonResponse = await response.Content.ReadAsStringAsync(token);
            SharedStatic.InstanceLogger.LogDebug($"[ArknightsNews] Response Body:\n{jsonResponse}");

            var rspBody = JsonSerializer.Deserialize(jsonResponse, ArknightsApiContext.Default.ArknightsBatchResponse);
            ...
            SharedStatic.InstanceLogger.LogDebug(
                $"[ArknightsNews] Parsed responses: News={_newsResponse != null}, Banner={_bannerResponse != null}, Sidebar={_sidebarResponse != null}");
            return 0;
        }
        catch (Exception ex)
        {
            SharedStatic.InstanceLogger.LogError($"[ArknightsNews] Failed to init news: {ex}");
            return -1;
        }
    }
    ...
    {
        SharedStatic.InstanceLogger.LogDebug($"[ArknightsNews] Downloading asset: {fileUrl}");
        try
        {
            await base.DownloadAssetAsyncInner(ApiDownloadHttpClient, fileUrl, outputStream, fileChecksum,
                downloadProgress, token);
            SharedStatic.InstanceLogger.LogDebug(
                $"[ArknightsNews] Download COMPLETED: {fileUrl} (Size: {outputStream.Length} bytes)");
        }
        catch (Exception ex)
        {
            SharedStatic.InstanceLogger.LogError($"[ArknightsNews] Download FAILED: {fileUrl}\nException: {ex}");
        }
    }
    ...
    private ArknightsCommonReq CreateCommonReq()
    {
        return new ArknightsCommonReq
        {
            AppCode = _appCode,
            Channel = _channel,
    ...
    {
        public ArknightsAnnouncement Item;
